...
export default function AchievementsForm() {
  ...
  const update = (updated) => setPortfolioData((prev) => ({ ...prev, achievements: updated }));
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Awards, certs, and accomplishments</p>
        </div>
        <button onClick={add}
      ...
      {achievements.map((item, i) => (
        <div key={i} className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-3">
          ...
          <div className="flex gap-2">
            {["certification", "achievement", "award"].map((type) => (
              <button key={type} onClick={() => handleChange(i, "type", type)}
                className={`flex-1 py-1.5 text-xs font-medium rounded-lg border transition capitalize ${
                  item.type === type
                    ? "border-indigo-500 bg-indigo-50 text-indigo-700"
                    : "border-gray-200 text-gray-500 hover:border-gray-300"
                }`}>
                {type === "certification" ? "🎓" : type === "achievement" ? "⭐" : "🏆"} {type}
              </button>
            ))}
          </div>
          ...
          <div>
            ...
            <input type="text" value={item.title} onChange={(e) => handleChange(i, "title", e.target.value)}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="text" value={item.issuer} onChange={(e) => handleChange(i, "issuer", e.target.value)}
                placeholder="Amazon Web Services" className={inputClass} />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Date</label>
              <input type="text" value={item.date} onChange={(e) => handleChange(i, "date", e.target.value)}
          ...
          <div>
            ...
            <textarea value={item.description} onChange={(e) => handleChange(i, "description", e.target.value)}
          ...
          <div>
            ...
            <input type="url" value={item.link} onChange={(e) => handleChange(i, "link", e.target.value)}
